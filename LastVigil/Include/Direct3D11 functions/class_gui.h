...
al32 struct CLASS_GUI {
	...
	void _Initialise(void) {	///- Add input variables for setting max values (-1 for defaults)
		...
		bufDGS           = buf->CreateStructured(0, element_dgs, sizeof(GUI_EL_DGS), MAX_GUI_ENTRIES, USAGE_DYNAMIC);
	...
	void _Deinitialise(void) {
		...
		if(element) _aligned_free(element);
		if(element_dgs) _aligned_free(element_dgs);
//		if(vertexRunLength) _aligned_free(element_dgs);
		if(textBuffer) _aligned_free(textBuffer);
		for(i = MAX_ATLAS; i; i++)
			if(siAtlasTexIndex[i])
				tex->Unload2D(siAtlasTexIndex[i]);
//		_aligned_free(vertex_index);
		for(i = siInterfaces; i; i++)
			if(interfaceProfile[i].vertex)
				_aligned_free(interfaceProfile[i].vertex);
		_aligned_free(interfaceProfile);
		_aligned_free(stAtlas);
		_aligned_free(stLanguage);
		_aligned_free(alphabet);
		_aligned_free(alphabet_pIMM);
		_aligned_free(spriteLib);
...
///--- To do...
	...
	cui32 CreateAlphabet(cwchptrc languageName, csi32 xSegments, csi32 ySegments, cfl32 charWidth, si16 alphabetIndex, csi16 atlasIndex) {
		...
		for(si32 iY = 0; iY < ySegments; iY++)
			for(si32 iX = 0; iX < xSegments; iX++) {
				...
				alphabet_pIMM[pIMMos + i].width = (i ? charWidth : 0.0f);
...
///- To do...
	...
	inline VEC2Df _CalculateOrigin(csi16 firstVert, csi16 curVert, csi16 alphabetIndex) {
		...
		cfl32   fScale       = element_dgs[firstVert].size.x;// * ScrRes.dims[ScrRes.state].aspect;
		cui32   uiBufIndex   = element_dgs[uiPrevVert].textArrayOS & 0x0FFFFFFF;
		fl32    fOffset      = 0.0f;

		for(ui8 i = 0; i < 32 && textBuffer[uiBufIndex + i]; i++) {
			cui32 uiCharIndex = alphabet[alphabetIndex].pIMMos + textBuffer[uiBufIndex + i];

			fOffset += alphabet_pIMM[uiCharIndex].width;
		}

		cfl32 uiXoffset = fOffset * fScale + uiPrevOrigin.x;

		return { uiXoffset, uiPrevOrigin.y };
	};

	inline cVEC4Df CalcActiveCoords(cVEC2Df viewPos, cVEC2Df spriteSize, cVEC2Df activeSize, ui8 bitField) {
...
//		cfl32  xSize  = activeSize.x * ScrRes.dims[ScrRes.state].aspect;
	...
	cui32 CreateText(cchptrc text, csi16 alphabetIndex, cVEC2Df viewPos, cVEC2Df size, cVEC4Df colour, ui8 alignment) {
		...
		cVEC2Df activeSize = { alphabet_pIMM[alphabet[alphabetIndex].pIMMos + 1].width * fl32(strlen(text)) * size.x, size.y };
		...
		for(si32 i = 0; i < siTextBlockSize; siGUIVerts++, i += 32) {
			...
			element_dgs[siGUIVerts].bitField      = 0x050;
		}
	...
	// Returns element index and vertex count
	cui32 CreateText(cchptrc text, csi32 charCount, csi16 alphabetIndex, cVEC2Df viewPos, cVEC2Df size, cVEC4Df colour, ui8 alignment) {
		if(siGUIElements >= MAX_GUI_ELEMENTS) return { 0x080000001 };
		...
		// Calculate width of string view space   ---   Change to iterate & add unique character widths
		cVEC2Df activeSize = { alphabet_pIMM[alphabet[alphabetIndex].pIMMos + 1].width * fl32(charCount) * size.x, size.y };
		...
		for(element_dgs[siFirstVert].origin[0] = viewPos; i < siTextLength; siGUIVerts++, i += 32) {
			...
			element_dgs[siGUIVerts].atlasIndex    = (ui8)alphabet[alphabetIndex].atlasIndex;
			element_dgs[siGUIVerts].elementType   = 0;
			element_dgs[siGUIVerts].bitField      = alignment << 4;
		}
		for(; i < siTextBlockSize; siGUIVerts++, i += 32) {
			element_dgs[siGUIVerts].origin[0]     = { 0, 0 };
			element_dgs[siGUIVerts].origin[1]     = { 0, 0 };
			element_dgs[siGUIVerts].origin[2]     = { 0, 0 };
			element_dgs[siGUIVerts].origin[3]     = { 0, 0 };
			element_dgs[siGUIVerts].colour        = Fix16x4x4((SSE4Df32 &)colour);
			element_dgs[siGUIVerts].size          = size;
			element_dgs[siGUIVerts].rotAngle      = 0.0f;
			element_dgs[siGUIVerts].parentIndex   = 0x0FFFFFFFF;
			...
			element_dgs[siGUIVerts].atlasIndex    = (ui8)alphabet[alphabetIndex].atlasIndex;
			element_dgs[siGUIVerts].elementType   = 0;
			element_dgs[siGUIVerts].bitField      = 0x08 | (alignment << 4);
		}
		csi16 siVertexCount = max(1, (siTextSize + 31) >> 5);
		siTextBufferOS += siTextBlockSize;
		element[siGUIElements].vertexCount[0] = ui16(siVertexCount);
		element[siGUIElements].vertexCount[1] = ui16(siGUIVerts - siFirstVert);

		return (ui32)siGUIElements++;
	}

	cui32 CreateCursor(cui16 spriteLibIndex, cui16 spriteIndex, cVEC2Df viewPos, cVEC2Df size, cVEC4Df colour, ui8 alignment) {
		if(siGUIElements >= MAX_GUI_ELEMENTS) return 0x080000001;
	...
	cui32 CreatePanel(cui16 spriteLibIndex, cui16 spriteIndex, cVEC2Df viewPos, cVEC2Df size, cVEC4Df colour, ui8 alignment) {
		if(siGUIElements >= MAX_GUI_ELEMENTS) return 0x080000001;
	...
	// Button's sprite index for it's alternate state == [spriteIndex + 1]
	cui32 CreateButton(cui16 spriteLibIndex, cui16 spriteIndex, cVEC2Df viewPos, cVEC2Df size, cVEC4Df colour, ui8 alignment) {
		if(siGUIElements >= MAX_GUI_ELEMENTS) return 0x080000001;
	...
	// Button's sprite index for it's alternate state == [spriteIndex] + 1
	cui32 CreateToggle(cui16 spriteLibIndex, cui16 spriteIndex, cVEC2Df viewPos, cVEC2Df size, cVEC4Df colour, ui8 alignment) {
		if(siGUIElements >= MAX_GUI_ELEMENTS) return 0x080000001;
		...
		element[siGUIElements].viewScale         = { 2.0f / activeSize.x, 2.0f / activeSize.y };
		element[siGUIElements].vertexIndex       = siGUIVerts;
		element[siGUIElements].elementType       = 2;	// bit7==Button "toggled"
		element[siGUIElements].func.hover[0]     = 0;
		element[siGUIElements].func.hover[1]     = 0;
	...
	// Returns index for panel. Index for indicator == [return value] + 1
	cui32 CreateScalar(cui16 spriteLibIndex, cui16 spriteIndexPanel, cui16 spriteIndexIndicator, cVEC2Df viewPos, cVEC2Df size, cVEC4Df colour, ui8 alignment) {
		if(siGUIElements >= MAX_GUI_ELEMENTS) return 0x080000001;
		...
		element[siGUIElements].vertexCount[0]    = element[siGUIElements].vertexCount[1] = 2;
		element[siGUIElements].vertexIndex       = siGUIVerts;
		element[siGUIElements].elementType       = 3;
		element[siGUIElements].soundBankIndex    = -1;
		...
		element_dgs[siGUIVerts++].bitField    = alignment << 4;
		element_dgs[siGUIVerts].viewPos       = { 0.0f, 0.0f };
		...
		element_dgs[siGUIVerts].atlasIndex    = (ui8)curLib.atlasIndex;
		element_dgs[siGUIVerts].elementType   = 3;
		element_dgs[siGUIVerts++].bitField    = 0x02;

		return siGUIElements++;
	}

	// Returns indices of panel element and text overlay
	cVEC2Du32 CreateTextBox(cchptrc text, csi16 alphabetIndex, cVEC2Df textSize, cVEC4Df textColour, cui16 spriteLibIndex, cui16 spriteIndex, cVEC2Df panelSize, cVEC4Df panelColour, cVEC2Df viewPos, ui8 panelAlignment, ui8 textAlignment) {
		if(siGUIElements >= MAX_GUI_ELEMENTS - 1) return { 0x080000001, MAX_GUI_ELEMENTS };

		cui32 panel   = CreatePanel(spriteLibIndex, spriteIndex, viewPos, panelSize, panelColour, panelAlignment);
		cui32 overlay = CreateText(text, alphabetIndex, { 0.0f, 0.0f }, textSize, textColour, textAlignment);
	...
	// Returns indices of panel element and text overlay
	cVEC2Du32 CreateTextBox(cchptrc text, csi32 charCount, csi16 alphabetIndex, cVEC2Df textSize, cVEC4Df textColour, cui16 spriteLibIndex, cui16 spriteIndex, cVEC2Df panelSize, cVEC4Df panelColour, cVEC2Df viewPos, ui8 panelAlignment, ui8 textAlignment) {
		if(siGUIElements >= MAX_GUI_ELEMENTS - 1) return { 0x080000001, MAX_GUI_ELEMENTS };

		cui32 panel   = CreatePanel(spriteLibIndex, spriteIndex, viewPos, panelSize, panelColour, panelAlignment);
		cui32 overlay = CreateText(text, charCount, alphabetIndex, { 0.0f, 0.0f }, textSize, textColour, textAlignment);
	...
	// Returns indices of panel element, text overlay, and cursor element
	cVEC3Du32 CreateInputBox(cchptrc text, csi32 charCount, csi16 alphabetIndex, cVEC2Df textSize, cVEC4Df textColour, cui16 spriteLibIndex, cui16 panelSpriteIndex, cui16 cursorSpriteIndex, cVEC2Df panelSize, cVEC2Df cursorSize, cVEC4Df panelColour, cVEC4Df cursorColour, cVEC2Df viewPos, ui8 panelAlignment, ui8 textAlignment) {
		if(siGUIElements >= MAX_GUI_ELEMENTS - 2) return { 0x080000001, MAX_GUI_ELEMENTS, 0 };

		cui32   panel   = CreatePanel(spriteLibIndex, panelSpriteIndex, viewPos, panelSize, panelColour, panelAlignment);
		cui32   overlay = CreateText(text, charCount, alphabetIndex, { 0.0f, 0.0f }, textSize, textColour, textAlignment);
		...
		cui32   cursor = CreateCursor(spriteLibIndex, cursorSpriteIndex, curPos, cursorSize, cursorColour, 0x0);

		element[panel].elementType = 4;
...
///--- To do...
	...
	inline void UpdateText(cchptr text, ui32 elementIndex) {
		...
		for(ui8 i = 0; i < element[elementIndex].vertexCount[0]; i++) {
			...
			element_dgs[offset].bitField &= 0x0FFFFFFF7;
		...
		for(ui8 i = textBlockSize; i < element[elementIndex].vertexCount[1]; i++)
			element_dgs[vertexIndex + i].bitField |= 0x08;
	}
	...
	inline void UpdateText(cchptr text, cVEC2Du32 elementIndex) {
		...
		if(textBlockSize >= element[elementIndex.x].vertexCount[1]) {
			element[elementIndex.x].vertexCount[0] = element[elementIndex.x].vertexCount[1];
			const size_t truncBlock = (size_t(element[elementIndex.x].vertexCount[1]) << 5) - 1;
			strncpy(&textBuffer[textBufferOS], text, truncBlock);
			textBuffer[textBufferOS + truncBlock] = 0;
		} else {
			element[elementIndex.x].vertexCount[0] = textBlockSize;
			strncpy(&textBuffer[textBufferOS], text, stringLength);
			textBuffer[textBufferOS + stringLength] = 0;
			csi32 deltaIndex = (textBufferOS + stringLength) & 0x01F;
			if(deltaIndex && deltaIndex < 24) {
		...
		for(ui8 i = 0; i < element[elementIndex.x].vertexCount[0]; i++) {
			csi32 offset = vertexIndex + i;
			csi32 step   = stringLength - (i << 5);
			element_dgs[offset].bitField &= 0x0FFFFFFF7;
			element_dgs[offset].origin[0] = _CalculateOrigin(vertexIndex, offset, element_dgs[offset].alphabetIndex);
		...
		for(ui8 i = textBlockSize; i < element[elementIndex.x].vertexCount[1]; i++)
			element_dgs[vertexIndex + i].bitField |= 0x08;
	}
