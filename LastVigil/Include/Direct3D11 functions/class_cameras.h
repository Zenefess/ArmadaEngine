...
al32 struct CLASS_CAMERAS {
	...
	CLASS_CAMERAS(void) { ptrLib[1] = this; }

	void SetDims(cVEC3Ds16 dims, ui8 cam) { s[cam].iDims[0].vector = { dims.x, dims.y, dims.z, 1 }; }

	void SetDims(cVEC4Ds16 dims, ui8 cam) { s[cam].iDims[0].vector = { dims.x, dims.y, dims.z, dims.w }; }

	void SetDimsf(cVEC3Ds16 dims, ui8 cam) { s[cam].fDims[0].vector = { float(dims.x), float(dims.y), float(dims.z), 1.0f }; }

	void SetDimsf(cVEC4Ds16 dims, ui8 cam) { s[cam].fDims[0].vector = { float(dims.x), float(dims.y), float(dims.z), float(dims.w) }; }

	void SetDimsf(cVEC3Ds16 chunkDims, cVEC3Ds16 chunkCounts, ui8 cam) {
		s[cam].fDims[0].vector = { float(chunkDims.x), float(chunkDims.y), float(chunkDims.z), 1.0f };
		s[cam].fDims[1].vector = { float(chunkCounts.x), float(chunkCounts.y), float(chunkCounts.z), 1.0f };
	}

	void SetDimsf(cVEC4Ds16 chunkDims, cVEC4Ds16 chunkCounts, ui8 cam) {
		s[cam].fDims[0].vector = { float(chunkDims.x), float(chunkDims.y), float(chunkDims.z), float(chunkDims.w) };
		s[cam].fDims[1].vector = { float(chunkCounts.x), float(chunkCounts.y), float(chunkCounts.z), float(chunkDims.w) };
	}
	...
	inline void SetCameraPos(float x, float y, float z, ui8 cam) { s[cam].fXpos = x; s[cam].fYpos = y; s[cam].fZpos = z; }
	inline void SetCameraPos(double x, double y, double z, ui8 cam) { s[cam].dXpos = x; s[cam].dYpos = y; s[cam].dZpos = z; }

	inline void SetCameraRot(float x, float y, float z, ui8 cam) { s[cam].fXrot = x; s[cam].fYrot = y; s[cam].fZrot = z; }
	inline void SetCameraRot(double x, double y, double z, ui8 cam) { s[cam].dXrot = x; s[cam].dYrot = y; s[cam].dZrot = z; }

	inline void MoveCameraUpY(cfl32 distance, cui8 cam) { s[cam].fYpos += distance; }
	inline void MoveCameraUpY(cfl64 distance, cui8 cam) { s[cam].dYpos += distance; }

	inline void MoveCameraRightXZ(cfl32 distance, cui8 cam) {
		s[cam].fXpos += cosf(s[cam].fYrot) * distance;
		s[cam].fZpos -= sinf(s[cam].fYrot) * distance;
	}
	inline void MoveCameraRightXZ(cfl64 distance, cui8 cam) {
		s[cam].dXpos += cos(s[cam].dYrot) * distance;
		s[cam].dZpos -= sin(s[cam].dYrot) * distance;
	}

	inline void MoveCameraForwardXZ(cfl32 distance, cui8 cam) {
		s[cam].fXpos += sinf(s[cam].fYrot) * distance;
		s[cam].fZpos += cosf(s[cam].fYrot) * distance;
	}
	inline void MoveCameraForwardXZ(cfl64 distance, cui8 cam) {
		s[cam].dXpos += sin(s[cam].dYrot) * distance;
		s[cam].dZpos += cos(s[cam].dYrot) * distance;
	}
	...
	inline void MoveCamera2DVectorXZ(VEC2Df distance, cui8 cam) {
		...
		cfl32 fSinY = sinf(s[cam].fYrot);
		cfl32 fCosY = cosf(s[cam].fYrot);
		s[cam].fXpos += fSinY * distance.y + fCosY * distance.x;
		s[cam].fZpos += fCosY * distance.y - fSinY * distance.x;
	}
	inline void MoveCamera2DVectorXZ(VEC2Dd distance, cui8 cam) {
		//cfl64 dXZsq = distance.x * distance.x + distance.y * distance.y;
		if(!(distance.x + distance.y)) return;
		cfl64 dSinY = sin(s[cam].dYrot);
		cfl64 dCosY = cos(s[cam].dYrot);
		s[cam].dXpos += dCosY * distance.x + dSinY * distance.y;
		s[cam].dZpos += dCosY * distance.y - dSinY * distance.x;
	}

	// Projections 2 & 3 use square aspect ratio
	inline void TransformProjections(cui8 cam) {
		mProj[0] = DX::XMMatrixPerspectiveLH(s[cam].fZoom * s[cam].fWidth, s[cam].fZoom * s[cam].fHeight, s[cam].fNearZ, s[cam].fFarZ);
		mProj[1] = DX::XMMatrixOrthographicLH(s[cam].fSize * ScrRes.dims[ScrRes.state].aspectI, s[cam].fSize, s[cam].fNearZ, s[cam].fFarZ);
		mProj[2] = DX::XMMatrixPerspectiveLH(s[cam].fZoom, s[cam].fZoom, s[cam].fNearZ, s[cam].fFarZ);
		mProj[3] = DX::XMMatrixOrthographicLH(s[cam].fSize, s[cam].fSize, s[cam].fNearZ, s[cam].fFarZ);
	}

	inline void TransformCamera(cui8 cam, cui8 proj) {
		cfl32x4 vCamPos    = DX::XMVectorSet(s[cam].fXpos, s[cam].fYpos, s[cam].fZpos, 0.0f);
		cmatrix mCamRot    = DX::XMMatrixRotationRollPitchYaw(s[cam].fXrot, s[cam].fYrot, -s[cam].fZrot);
		...
		s[cam].frustum.fLeft   = { .xmm = _mm_add_ps(m[1].xmm1, m[0].xmm0) };
		s[cam].frustum.fBottom = { .xmm = _mm_add_ps(m[1].xmm1, m[0].xmm1) };
		s[cam].frustum.fRight  = { .xmm = _mm_sub_ps(m[1].xmm1, m[0].xmm0) };
		s[cam].frustum.fTop    = { .xmm = _mm_sub_ps(m[1].xmm1, m[0].xmm1) };
		s[cam].frustum.fNear   = { .xmm = _mm_add_ps(m[1].xmm1, m[1].xmm0) };
		s[cam].frustum.fFar    = { .xmm = _mm_sub_ps(m[1].xmm1, m[1].xmm0) };

		for(ui8 i = 0; i < 6; i++) {
			cfl32 magRCP = 1.0f / V3Magnitute(s[cam].frustum.fPlane[i].xmm);
			V3NormaliseR(s[cam].frustum.fPlane[i].xmm, magRCP);
			s[cam].frustum.fPlane[i].length *= magRCP;
		}
	}
	...
	inline cfl32 DistanceFromCamera(csi128 target, cui8 cam) {
		...
		cfl32x4 diff = { fTarget.m128_f32[0] - s[cam].fXpos, fTarget.m128_f32[0] - s[cam].fYpos, fTarget.m128_f32[0] - s[cam].fZpos };
	...
	inline cfl32 DistanceFromCamera(cfl32x4 target, cui8 cam) {
		cfl32x4 diff = { target.m128_f32[0] - s[cam].fXpos, target.m128_f32[0] - s[cam].fYpos, target.m128_f32[0] - s[cam].fZpos };
	...
	// Return values are 0x080000001 if target is beyond horizon
	inline cVEC2Ds32 CursorLayerIntersect(csi32 layer, cfl32x4 camOffset, cVEC2Ds32 curPos, cui8 cam, cui8 proj) {
		...
		cfl32x4 vRayOri = _mm_add_ps(s[cam].pos32.xmm, camOffset);
	...
	inline cui8 SphereFrustumIntersect2(cAVX8Df32 spheres, cui8 cam) {
		...
		for (i = 0; i < 6; i++)
			if(_mm_dp_ps(spheres.xmm0, s[cam].frustum.fPlane[i].xmm, 0x0F1).m128_f32[0] < 0) { success ^= 0x01; break; }
		for (i = 0; i < 6; i++)
			if(_mm_dp_ps(spheres.xmm1, s[cam].frustum.fPlane[i].xmm, 0x0F1).m128_f32[0] < 0) { return success ^ 0x02; }
	...
	inline cui8 SphereFrustumIntersect8(cSSE4Df32 (&spheres)[8], cui8 cam) {
		PLANEfl32 (&plane)[6] = s[cam].frustum.fPlane;
		ui8 i, success = 0xFF;
	...
	inline cui8 ChunkFrustumIntersect2_(cAVX8Ds32 chunks, cui8 cam, cui8 proj) {
		...
		cfl32 depthOS      = s[cam].fDims[1].vector.z * 0.5f;
		cfl32 boundingSize = max(max(s[cam].fDims[0].vector.x, s[cam].fDims[0].vector.y), s[cam].fDims[0].vector.z) * 0.86602540378443864676372317075294f;

		cAVX8Df32 chunkOffsets = { .vector = { { 0.5f, 0.5f, depthOS + 0.5f, 1.0f }, { 0.5f, 0.5f, depthOS + 0.5f, 1.0f } } };
		cAVX8Df32 chunkScale   = { .xmm = { s[cam].fDims[0].xmm, s[cam].fDims[0].xmm } };
		...
		cVEC4Df antiSize = { s[cam].fZoom - radius.vector.x, s[cam].fZoom - radius.vector.y,
									s[cam].fZoom - radius.vector.z, s[cam].fZoom - radius.vector.w };
...
//		cui8 result = (fabsf(rayOrigins.vector.x) > s[cam].fWidth || fabsf(rayOrigins.vector.y) > s[cam].fHeight ? 0 : 0x01) |
//						  (fabsf(rayOrigins.vector.z) > s[cam].fWidth || fabsf(rayOrigins.vector.w) > s[cam].fHeight ? 0 : 0x02);
//		cui8 result = (rayOrigins.vector.x > s[cam].fWidth || rayOrigins.vector.y > s[cam].fHeight ? 0 : 0x01) |
//						  (rayOrigins.vector.z > s[cam].fWidth || rayOrigins.vector.w > s[cam].fHeight ? 0 : 0x02);
	...
	inline cui8 ChunkFrustumIntersect2(cAVX8Ds32 chunks, cui8 cam) {
		...
		for (i = 0; i < 6; i++) {
			cfl32x4 plData = s[cam].frustum.fPlane[i].xmm;
	...
	inline cui8 ChunkFrustumIntersect8(SSE4Ds32 chunk, cui8 cam) {
		...
		for(i = 0; i < 8; i += 2, chunk.vector.x++) {
			...
			for (j = 0; j < 6; j++) {
				cfl32x4 plData = s[cam].frustum.fPlane[j].xmm;
				cfl32   side   = _mm_dp_ps(chunkOrigins.xmm0, plData, 0x0F1).m128_f32[0];
				if (side < boundingSize) {
					success ^= 0x01 << i;
					break;
...
/*
	...
	inline cui8 Test8EntityVisibility(cui32 entityIndex, cui8 cam) {
		cfl32 boundingSize = Max(s[cam].fDims[0].vector) * -0.86602540378443864676372317075294f;
