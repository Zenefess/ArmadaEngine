...
//	     CHAR							 stShaderModel[3][7] { "vs_5_0", "gs_5_0", "ps_5_0" };
	...
	cui32 CreateVertexFormat(si8 profileIndex, si8 vertexShader, si8 inputFormat) {
		...
		Try(stCreateIL, dev->CreateInputLayout(ied[inputFormat], numEntries[inputFormat], pBlob[1][vertexShader]->GetBufferPointer(),
															pBlob[1][vertexShader]->GetBufferSize(), &pIL[i]));
